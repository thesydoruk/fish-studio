...
class JsonSynthesisRequest(BaseModel):
    """JSON synthesis body. First ``speaker_wav_b64`` is also the timing slot target."""
...
def _header_safe(value: str) -> str:
    """HTTP headers are Latin-1; strip control chars so the WAV body stays intact."""
    cleaned = value.replace("\r", " ").replace("\n", " ").strip()
    return cleaned.encode("latin-1", "replace").decode("latin-1")


def synthesis_response_headers(result: SynthesisResult) -> dict[str, str]:
    """Attach sample-rate, language, and a warning when the take is still broken."""
    headers = {
...
def create_app(settings: ServerSettings) -> FastAPI:
    ...
    async def synthesize_multipart(
        ...
        return Response(
            ...
            headers=synthesis_response_headers(result),
        )
