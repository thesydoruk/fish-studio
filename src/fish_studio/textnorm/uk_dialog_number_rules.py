"""Corpus-derived Ukrainian number rules for dialogue TTS.
...
class NumberRule:
    ...
    corpus_hits: int
    """Approximate token hits on source dialogue lines with digits."""
...
_RULES: tuple[NumberRule, ...] = (
    ...
    NumberRule(
        ...
        pattern=re.compile(
            r"(?i)(?<![\d.,])(?P<num>\d+(?:[.,]\d+)?)\s*(?:%|відсотк(?:ів|и|а|у)?\b)"
        ),
    ...
    NumberRule(
        id="duration_unit",
        description="Count + time unit (years/hours/minutes/days/weeks), incl. decimals.",
        pattern=re.compile(
            r"(?i)(?<![\d.,])(?P<num>\d{1,4}(?:[.,]\d+)?)\s*"
            r"(?P<unit>хв\.?|хвилин(?:и|у|ами)?|сек(?:унд(?:и|у|ами)?)?|"
            r"годин(?:и|у|ами)?|дн(?:і|ів|я)|день|тижн(?:і|ів|я)|"
            r"рок(?:и|ів|у|ом)|місяц(?:і|ів|я|ь))\b"
        ),
        expand="Cardinal/decimal agreeing with unit "
        "(200 років → двісті років; 5,02 секунди → п'ять цілих дві сотих секунди).",
        corpus_hits=203,
        examples=(
    ...
    NumberRule(
        ...
        pattern=re.compile(
            r"(?i)(?<![\d.,])(?P<num>-?\d{1,3}(?:[.,]\d+)?)\s*"
            r"(?:°|градус(?:и|ів|а)?)\b"
        ),
        expand="Cardinal/decimal + agreeing 'градус' form; X,5 reads 'з половиною' "
        "(173,5 градуса → сто сімдесят три з половиною градуси); "
        "keep scale words (Фаренгейт) as-is.",
        corpus_hits=5,
        examples=(
    ...
    NumberRule(
        ...
        description="Remaining 19xx/20xx years without 'року' (titles, product names).",
