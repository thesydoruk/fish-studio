...
clock hours in dialogue lines. Not a general morphological engine.
"""
...
# Denominator by number of fraction digits: (singular, plural) feminine forms.
_FRACTION_DENOMS: dict[int, tuple[str, str]] = {
    1: ("десята", "десятих"),
    2: ("сота", "сотих"),
    3: ("тисячна", "тисячних"),
...
def decimal_words(whole: int, frac: str) -> str:
    """TTS-friendly decimal: ``2,74`` → ``дві цілих сімдесят чотири сотих``."""
    frac = frac or "0"
    num = int(frac)
    denoms = _FRACTION_DENOMS.get(len(frac), _FRACTION_DENOMS[6])
    denom = denoms[0] if num % 10 == 1 and num % 100 != 11 else denoms[1]
    ...
    return f"{whole_part} {cardinal(num, gender='fem')} {denom}"


def half_words(whole: int, *, gender: str = "masc") -> str:
    """``X,5`` as spoken halves: ``1,5`` → ``півтора``, ``3,5`` → ``три з половиною``."""
    if whole == 1:
        return "півтори" if gender == "fem" else "півтора"
    return f"{cardinal(whole, gender=gender)} з половиною"
