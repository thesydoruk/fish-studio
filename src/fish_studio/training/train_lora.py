...
from fish_studio.config import DEFAULT_LORA_TARGET_MODULES
...
def parse_args() -> argparse.Namespace:
    ...
    defaults: dict[str, object] = {
        ...
        "max_speaker_weight": 10000,
        "cross_lingual_prob": 0.7,
        "protos_dir": None,
        ...
        defaults.update(
            {
                ...
                "max_speaker_weight": ft.max_speaker_weight,
                "cross_lingual_prob": ft.cross_lingual_prob,
    ...
    parser.add_argument(
        ...
        help="Comma-separated LoRA targets. Slow names stay on the text tower; "
        "list fast_* explicitly to adapt the acoustic decoder / timbre.",
    )
    parser.add_argument(
        "--max-speaker-weight",
        type=int,
        default=defaults["max_speaker_weight"],
        help="Cap per-speaker sampling weight; 0 keeps raw clip counts",
    )
    parser.add_argument(
        "--cross-lingual-prob",
        type=float,
        default=defaults["cross_lingual_prob"],
        help="When a speaker has EN and UK clips, fraction of EN-ref → UK-target pairs",
    )
    parser.add_argument(
        "--resume",
...
def main() -> None:
    ...
    print(
        ...
        f"(max_group_weight={args.max_speaker_weight}, "
        f"cross_lingual_prob={args.cross_lingual_prob})"
    )
    ...
    overrides = [
        ...
        f"+train_dataset.max_group_weight={args.max_speaker_weight}",
        f"+train_dataset.cross_lingual_prob={args.cross_lingual_prob}",
        f"+val_dataset.max_group_weight={args.max_speaker_weight}",
        f"+val_dataset.cross_lingual_prob={args.cross_lingual_prob}",
