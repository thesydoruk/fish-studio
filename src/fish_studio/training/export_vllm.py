...
Output goes to ``{data_root}/training/fish/vllm/`` by default; point
``FISH_SPEECH_MODEL`` at it and restart the vLLM server.

Never run this while vLLM is serving the same output path: the process
memory-maps ``model.safetensors``, and an in-place overwrite can silently
corrupt cloning until restart. Stop vLLM first, or pass ``--output`` to a
fresh directory and retarget ``FISH_SPEECH_MODEL``.
"""
