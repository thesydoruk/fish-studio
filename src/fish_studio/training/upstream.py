...
def run_fish_merge(script_args: list[str]) -> None:
    ...
    # Upstream keeps the model (11 GB on s2-pro), deep-copies its state dict,
    # folds the adapter into a third copy for saving, then loads the saved
    # file back to diff it: past 30 GB, and the kernel killed the merge after
    # the model was already on disk. The copy becomes a shallow dict and the
    # post-save reload ends the process instead -- apply_merge_groups re-reads
    # that file against stock right after and would fail on a broken one.
    bootstrap = (
        "import json, os, sys; "
        ...
        "_load = upstream_merge.torch.load; "
        "upstream_merge.torch.load = (lambda path, *a, **k: "
        "(_ for _ in ()).throw(SystemExit(0)) if str(path).endswith('model.pth') "
        "else _load(path, *a, **k)); "
