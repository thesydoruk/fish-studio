...
class SegmentationConfig:
    ...
    # A LoRA learns how long an utterance runs, so the longest clip has to cover the
    # longest span synthesis will ask for in one generation: FISH_SPEECH_CHUNK_LENGTH
    # characters at roughly 14 chars/s. Train only on short clips and the adapter
    # learns to stop early, which shows up as long lines being cut off mid-sentence.
    max_duration_sec: float = 20.0
    target_duration_sec: float = 12.0
    min_speech_duration_sec: float = 0.5
    min_chars: int = 10
    max_chars: int = 320  # headroom over max_duration_sec so duration is the binding limit
    padding_sec: float = 0.05  # extra audio kept on each side of the aligned span
    sample_rate: int = 22050
    # Join adjacent same-speaker segments across gaps this short. Wide enough to
    # cross a sentence pause, so merging can actually reach max_duration_sec.
    ...
    # HF corpora ship clips padded with 1-2 s of digital silence on each side.
    # Training on those teaches the acoustic decoder to fade out mid-utterance.
    trim_silence: bool = True
    trim_silence_db: float = -50.0
    trim_silence_keep_sec: float = 0.05
