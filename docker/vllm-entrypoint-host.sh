#!/usr/bin/env bash
# Run vLLM from the host venv mounted at /venv (see docker-compose.yml).
set -euo pipefail
...
deploy_config="$("$PY" /app/scripts/generate_fish_deploy.py)"

IFS=$'\t' read -r model port gpu_util < <("$PY" - <<'PY'
import os
...
cmd=("$PY" /venv/bin/vllm serve "$model" --omni --port "$port" --gpu-memory-utilization "$gpu_util")
